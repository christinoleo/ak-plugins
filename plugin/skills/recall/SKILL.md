...
Transcripts live under `~/.claude/projects/<slug>/`, where `<slug>` is the workspace path with the leading slash dropped and each "/" turned into "-" (so `/home/you/proj` becomes `home-you-proj`). Every line of a `.jsonl` file there is one chat message.
