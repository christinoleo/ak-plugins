...
- `model`: `haiku` for narrow slices, `sonnet` for broad ones
...
After the explanation is complete, spawn three architectural critics in a single message. Diversity here comes from the assigned lens, not from the model, so give each critic a distinct angle: correctness and failure modes, layering and ownership, and evolution cost.

For each critic:
- `subagent_type`: `Explore`
- `model`: `opus`. Drop a critic to `sonnet` when the subsystem is small; escalate nothing below `opus` when the architecture is contested.
