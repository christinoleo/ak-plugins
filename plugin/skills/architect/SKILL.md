...
Naming a file isn't grounding. Produce the traced model `how` prescribes. If the design redefines ownership or layering, dig up why the existing shape is the way it is before you overrule it. Read the git history on the files you are about to restructure, the PRs that introduced them, and any ADRs or design notes the repo keeps. The rationale you find is a constraint. What you cannot find is a risk you should name in the sketch rather than assume away.

Skip Phase A only when the work is genuinely greenfield with no surrounding system to integrate.

## Phase B: Sketch

Spawn three design runners in a single message, each with `references/runner-prompt.md` as its prompt plus the Phase A grounding artifacts. Each produces a design package shaped per `references/rationale-template.md`: the caller's usage written first, then the type sketch, function signatures, module map, and prose rationale derived from it.

- `subagent_type`: `Explore`
- `model`: `opus`, or `fable` for one runner when you want a reader that argues differently
- Each runner writes to its own path (`/tmp/architect-<slug>/candidate-<n>/`). Three runners writing to one path is shared mutable state and fails the **separate-before-serializing-shared-state** principle skill.

Three is the number because divergence comes from the brief, not the count. Give each runner a different starting constraint so they cannot converge by accident: one optimizing for the smallest public surface, one for the data model first, one for the change that touches the fewest existing callers.
...
Read every candidate end to end before picking; skimming surfaces the one whose surface looks most familiar, not the best one. Pick the strongest as the base, graft the best ideas from the others into it, and record what you took from where. That record populates the rationale's "Synthesis decision" section. Candidates converging on one shape is a strong agreement signal, so ship the consensus and note it. Candidates diverging wildly means Phase A was under-specified, so reframe and re-run rather than averaging.
...
4. Return to Phase B and re-run the design runners.
