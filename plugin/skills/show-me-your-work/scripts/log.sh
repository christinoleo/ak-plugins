...
clean() {
	local v probe
	v=$(printf '%s' "$1" | tr '\t\n\r' '   ')
	# Check the formula prefix against the value with leading blanks removed.
	# A spreadsheet ignores them, so " =1+1" is still a formula, but a bare
	# case pattern on $v would let it through unquoted.
	probe="${v#"${v%%[![:space:]]*}"}"
	case "$probe" in
		=*|+*|-*|@*) printf "'%s" "$v" ;;
		*) printf '%s' "$v" ;;
