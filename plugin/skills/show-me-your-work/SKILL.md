...
At the end of the run, before handing back, check the log told the truth. Reconstruct what actually happened from this session's own history. Stay inside the active project: transcripts live under `~/.claude/projects/<project-slug>/`, and globbing across `~/.claude/projects/*/` reads unrelated private chats from other work. Walk the log against what actually happened:
...
Before handing back, spawn a reviewer subagent that did not do the work. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. Give it the audit trail and this session's history, and tell it to flag what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

Independence comes from the brief, not from the vendor. Assign the reviewer the skeptic's lens explicitly: its job is to find what the trail is quietly not saying. Run it on `opus`, or on `fable` when you want a reader that argues differently from the one that wrote the log.
...
Every reply for a run that produced a trail ends with an "Attention" section listing each flag, pointing to specific rows or moments. "No flags" is a valid outcome. The self-audit asks whether the log told the truth; this asks what the user should still scrutinize even when it did.
