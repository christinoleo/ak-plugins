...
Open a todolist whose first item is to read the `principle-*` skills that match the work in front of you. Then add the phases below as todos.
...
Present the framing and tradeoffs before committing to a long run. A multi-hour run earns one checkpoint before it starts.
