...
name: p2
description: |
  Spawns a trigger agent that drives THIS session through a GitHub issue epic: task → develop →
...
If `$ARGUMENTS` contains an epic issue number, use it as `EPIC_ID`.
...
Verify there are ready tasks belonging to this epic:
```bash
